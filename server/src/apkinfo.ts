...
function readZipEntry(buf: Buffer, wanted: string): Buffer {
  ...
  if (p === 0xffffffff) throw new Error("zip64 central directory is not supported");
...
function parseManifest(axml: Buffer): { versionName: string; versionCode: number } {
    ...
    if (type === CHUNK_STRING_POOL) {
      ...
      for (let a = 0; a < attrCount; a++) {
        ...
        if (resId === ATTR_VERSION_CODE) {
          ...
          if (rawValueIdx >= 0) versionName = strings[rawValueIdx];
          else if (dataType === 0x03) versionName = strings[data];
        }
      }
...
let cache: { key: string; info: ApkInfo } | null = null;
 ...
 */
export function readApkInfo(path: string): ApkInfo {
