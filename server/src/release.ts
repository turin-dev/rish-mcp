...
export interface Release extends ApkInfo {
  /** Release tag the bytes came from, or "local" for an APK_PATH override. */
  tag: string;
  path: string;
}
...
export interface ReleaseSourceOptions {
  ...
  apiBase: string;
  pollMs: number;
  /** Dev/test escape hatch: serve this file and never call GitHub. */
  localApk?: string;
}

export function releaseOptionsFromEnv(): ReleaseSourceOptions {
  return {
...
export class ReleaseSource {
  private current: Release | null = null;
  private timer: NodeJS.Timeout | null = null;

  constructor(private readonly opts: ReleaseSourceOptions) {}
  ...
  private loadCache(): void {
    try {
      if (!existsSync(this.apkFile) || !existsSync(this.metaFile)) return;
      const meta = JSON.parse(readFileSync(this.metaFile, "utf8")) as { tag?: string };
  ...
  async refresh(): Promise<void> {
    try {
      ...
      if (this.current?.tag === tag) return; // already have it

      const asset = (body.assets ?? []).find((a) => a.name.endsWith(".apk"));
      if (!asset) throw new Error(`release ${tag} has no .apk asset`);
      ...
      if (info.versionName !== tag.replace(/^v/, "")) {
        console.warn(
          `[release] tag ${tag} does not match APK versionName ${info.versionName}; trusting the APK`,
        );
      }
