...
export class OAuthProvider {
    ...
    router() {
        ...
        r.post("/oauth/register", (req, res) => {
            const ru = (req.body?.redirect_uris ?? []);
            ...
            const client_id = this.sign({ t: "client", ru: ru, iat: now() });
            res.status(201).json({
                client_id,
                redirect_uris: ru,
        ...
        const parseAuthReq = (q) => {
            ...
            if (!client.ru.includes(redirect_uri))
            ...
            if (!cc || String(q.code_challenge_method ?? "") !== "S256")
                return { error: "PKCE S256 code_challenge required" };
            return { client_id, redirect_uri, cc, state: q.state ? String(q.state) : undefined };
        };
        const hiddenInputs = (p) => Object.entries({
            response_type: "code",
        ...
        r.get("/oauth/authorize", (req, res) => {
            const p = parseAuthReq(req.query);
            if ("error" in p) {
        ...
        r.post("/oauth/token", (req, res) => {
            ...
            if (grant === "refresh_token") {
                ...
                if (rt?.t !== "rt")
                    return fail("invalid_grant", "bad or expired refresh_token");
...
function consentPage(hiddenInputs, error = "") {
    return `<!doctype html>
