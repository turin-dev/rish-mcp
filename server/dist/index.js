...
import { registry, normalizeKind } from "./relay.js";
...
function latestAgent() {
    ...
    if (r)
        return { version: r.versionName, versionCode: r.versionCode, source: "apk", release: r };
...
function buildMcpServer() {
    const server = new McpServer({ name: "rish-mcp", version: "0.5.0" }, { capabilities: { tools: {} } });
    server.registerTool("run_shell", {
        title: "Run a shell command on an Android device",
        description: "Executes a shell command on a connected Android phone, tablet, or Wear OS watch with Shizuku " +
        ...
        try {
            ...
            const body = `exit=${r.code} (${r.durationMs}ms)` +
...
httpServer.on("upgrade", (req, socket, head) => {
    ...
    wss.handleUpgrade(req, socket, head, (ws) => {
        ...
        registerAgent(ws, deviceId, name, sdk, kind, agentVersion, Number.isFinite(agentVersionCode) ? agentVersionCode : 0);
    });
});
function registerAgent(ws, deviceId, name, sdk, kind, agentVersion, agentVersionCode) {
    const device = {
        id: deviceId,
        name,
        sdk,
    ...
    if (agentVersionCode < latest.versionCode) {
        console.warn(`[agent] outdated: ${deviceId} runs ${agentVersion} (code ${agentVersionCode}); ` +
